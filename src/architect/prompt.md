...
- Each decision node must ask for exactly one atomic answer at the end of the available_info field — just a single action, choice, or association. Do NOT create a node whose correct answer is a list, a set of pairings, or several distinct facts (e.g. "match each of these four objects to its respective movie character", "name all three heroes", "name the correct god and its domain").
