...
1. **`"action_match"`** -- the user's message semantically matches one of the current node's `expected_actions` (if it's a decision node) or one or more `required_components` (if it's the accumulator node).
    ...
    - A literal `action_keyword` appearing verbatim in the user input  makes that action a *candidate* match -- but it is not, by itself, a guaranteed match. Before committing to it as a match, confirm that the overall *intent* of the user's message aligns with that action.
...
Edge cases:
- Ambiguous match: if the user's message could match an `expected_action` only weakly, prefer `"hint_needed"` over a forced match. A literal keyword hit is never ambiguous if the overall intent of the user's message aligns with the `expected_action`.
